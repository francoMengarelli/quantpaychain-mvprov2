...
export class AMLRulesEngine {
  private rules: Map<string, AMLRule>;
  ...
  public evaluateRules(
    ...
    customer: Customer,
    context?: any
  ): Array<{ rule: AMLRule; matched: boolean; action?: RuleAction }> {
    const results: Array<{ rule: AMLRule; matched: boolean; action?: RuleAction }> = [];
   ...
   */
  public addRule(rule: AMLRule): void {
    this.rules.set(rule.id, rule);
  }
   ...
   */
  public updateRule(ruleId: string, updates: Partial<AMLRule>): void {
   ...
   * Get rule by ID
   */
  public getRule(ruleId: string): AMLRule | undefined {
