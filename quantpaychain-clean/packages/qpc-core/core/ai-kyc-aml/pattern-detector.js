...
class PatternDetector {
     ...
     */
    async detectPatterns(transaction, customer, transactionHistory) {
        try {
            const matches = [];
            ...
            throw new errors_1.PatternDetectionError(`Pattern detection failed: ${error.message}`);
        }
    }
    /**
     * Evaluate a specific pattern
     */
    evaluatePattern(pattern, transaction, customer, transactionHistory) {
     ...
     */
    detectStructuring(transaction, history, indicators) {
        let confidence = 0;
        ...
        if (history && history.length > 5) {
            const recentSimilar = history.filter(t => Math.abs(t.amount - transaction.amount) < threshold * 0.1);
     ...
     */
    detectRoundTripping(transaction, history, indicators) {
        let confidence = 0;
        if (!history)
            return confidence;
        // Look for reciprocal transactions
        const reciprocal = history.find(t => t.sender.accountId === transaction.receiver.accountId &&
     ...
     */
    detectLayering(transaction, history, indicators) {
        let confidence = 0;
     ...
     */
    detectVelocity(transaction, customer, history, indicators) {
        let confidence = 0;
        if (!history)
            return confidence;
        // Check transaction frequency
        const last24Hours = Date.now() - (24 * 60 * 60 * 1000);
        const recentTxs = history.filter(t => new Date(t.timestamp).getTime() > last24Hours);
     ...
     */
    detectDormancyBreak(transaction, customer, history, indicators) {
        let confidence = 0;
